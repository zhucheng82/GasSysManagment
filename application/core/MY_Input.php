...
class MY_Input extends CI_Input {
	...
	public function posts($vers)
	{
		$posts = array();
		foreach ($vers as $ver) 
		{
			$posts[$ver] = $this->post($ver);
		}
		return $posts;
	}
	...
	{
		$val = parent::post($index, $xss_clean);
		if ($val ===FALSE) return '';
		return $this->filter( $val , $strip, FALSE);
	}
	...
	public function filter($str, $strip=TRUE, $is_urlencode=TRUE)
	{
		if (is_string($str))
		{
			...
			$str = $this->RemoveXSS( trim($str) );//htmlspecialchars
		}elseif (is_array($str))
		{
	 ...
	 * @return array
	 */
	function get_param($query, $uri='')
	{
		$page = 'page';
		if (is_array($query))
		{
			$numeric = array('page','cat','cata','catb','catc',
							'cid','sort','cida','cidb','cidc','amount',
							'dqa','dqb','dqc','type','level','psize',
							'zdq','zcid');
//			$pi		 = array();
			$string  = array('ky','tag');
			$ver 	 = array();
			$permitted_uri_chars = str_replace(array('\\-', '\-'), '-', preg_quote(config_item('permitted_uri_chars'), '-'));
			$uri 	 = str_replace('%2C', ',', $uri);
			foreach($query as $key)
			{
				if (in_array($key, $numeric) && preg_match('~'.$key.'([\d|.]+)~', $uri, $m))
				{
					$ver[$key] = $this->filter($m[1]);
				}
//				elseif (in_array($key, $pi) && preg_match('~'.$key.'([0-9,]+)~', $uri, $m))
//				{
//					$ver[$key] = $this->filter($m[1]);
//				}
				elseif (in_array($key, $string) && preg_match('|'.$key.'(['.$permitted_uri_chars.']+$)|i', $uri, $m))
				{
					$ver[$key] = $this->filter($m[1]);
				}
			}
