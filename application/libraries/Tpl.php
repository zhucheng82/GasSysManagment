...
class Tpl{
	 ...
	 * @param int $time
	 */
	public static function showpage($page_name='',$layout='', $tpl_dir='',$time=2000){
		$the = self::getInstance();
		...
		//默认是带有布局文件
		if (empty($layout)){
			$layout = $tpl_dir.'inc/'.self::$layout_file.'.php';
		}else {
			$layout = $tpl_dir.'inc/'.$layout.'.php';
		}
		$layout_file = VIEWPATH.'/'.$layout;
		$tpl_file = VIEWPATH.'/'.$tpl_dir.$page_name.'.php';
		...
		if (file_exists($tpl_file)){
			...
			if ($layout != ''){
				if (file_exists($layout_file)){
					...
					$error = 'Tpl ERROR:'.$layout.' is not exists';
			...
			$error = 'Tpl ERROR:'.$tpl_dir.$page_name.'.php'.' is not exists.';
			throw_exception($error);
		}
	}
	/**
