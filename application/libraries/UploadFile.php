...
class UploadFile{
	...
	public function upfile($field){
		...
		if ($this->upload_file['tmp_name'] == ""){
			$this->setError(lang('cant_find_temporary_files'));
			return false;
		}
		...
		if(!is_uploaded_file($this->upload_file['tmp_name'])){
			$this->setError(lang('upload_file_attack'));
			return false;
		}

		//验证文件大小
		if ($this->upload_file['size']==0){
			$error = lang('upload_file_size_none');
			$this->setError($error);
			return false;
		}
		if($this->upload_file['size'] > $this->max_size*1024){
			$error = lang('upload_file_size_cant_over').$this->max_size.'KB';
			$this->setError($error);
			return false;
		}
		...
		if(!in_array($this->ext,$this->allow_type)){
			$error = lang('image_allow_ext_is').implode(',',$this->allow_type);
			$this->setError($error);
			return false;
		}
		...
		if(!$image_info = @getimagesize($this->upload_file['tmp_name'])) {
			$error = lang('upload_image_is_not_image');
			$this->setError($error);
			return false;
		}
...
//				}
		...
		if(@move_uploaded_file($this->upload_file['tmp_name'],BASE_UPLOAD_PATH.'/'.$this->save_path.'/'.$this->file_name)){
			//产生缩略图
			if ($ifresize){
				$CI =& get_instance();
				$CI->load->library('ResizeImage');
				$resizeImage	= new ResizeImage();
				$save_path = rtrim(BASE_UPLOAD_PATH.'/'.$this->save_path,'/');
				for ($i=0;$i<count($thumb_width);$i++){
			...
			//删除原图
			if ($this->ifremove && is_file(BASE_UPLOAD_PATH.'/'.$this->save_path.'/'.$this->file_name)) {
				@unlink(BASE_UPLOAD_PATH.'/'.$this->save_path.'/'.$this->file_name);
			}
			return true;
		}else {
			$this->setError(lang('upload_file_fail'));
			return false;
		}
...
//				$resizeImage->newImg($save_path.'/'.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling);
	...
	private function fileInputError(){
		switch($this->upload_file['error']) {
			...
			case 1:
				...
				$this->setError(lang('upload_file_size_over'));
				return false;
				break;
			...
			case 3:
				//文件只有部分被上传
				$this->setError(lang('upload_file_is_not_complete'));
				return false;
				break;

			case 4:
				//没有文件被上传
				$this->setError(lang('upload_file_is_not_uploaded'));
				return false;
				break;

			case 6:
				//找不到临时文件夹
				$this->setError(lang('upload_dir_chmod'));
				return false;
				break;

			case 7:
				//文件写入失败
				$this->setError(lang('upload_file_write_fail'));
				return false;
				break;
	...
	public function setPath(){
		 ...
		 * 判断目录是否存在，如果不存在 则生成
		 */
		if (!is_dir(BASE_UPLOAD_PATH.'/'.$this->default_dir)){
		...
		//设置权限
		@chmod(BASE_UPLOAD_PATH.'/'.$this->default_dir,0755);

		//判断文件夹是否可写
        if(!is_writable(BASE_UPLOAD_PATH.'/'.$this->default_dir)) {
			$this->setError(lang('upload_file_dir').$this->default_dir.lang('upload_file_dir_cant_touch_file'));
			return false;
		}
