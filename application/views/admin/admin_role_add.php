...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <li><a href="JavaScript:void(0);" class="current"><span><?php echo lang('admin_add_limit_gadmin');?></span></a></li>
      </ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form id="add_form" method="post" action="<?php echo ADMIN_SITE_URL.'/admin/role_save'?>">
    ...
    <table class="table tb-type2 nobdb">
      <tbody>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" id="role_name" maxlength="40" name="role_name" class="txt"></td>
                  ...
                  <input id="limit<?php echo $k;?>" type="checkbox" onclick="selectLimit('limit<?php echo $k;?>')">
                    <label for="limit<?php echo $k;?>"><b><?php echo $v['name'];?></b>&nbsp;&nbsp;</label>
                      <?php foreach($v['child'] as $xk => $xv) { ?>
                        <label><input nctype='limit' class="limit<?php echo $k;?>" type="checkbox" name="permission[]" value="<?php echo $xv['act'];?>">
	...
	$("#add_form").validate({
        ...
        rules : {
            role_name : {
                required : true,
                    remote	: {
                    url :'<?php echo ADMIN_SITE_URL?>/admin/ajax',
                    type:'get',
                    data:{
                    	role_name : function(){
