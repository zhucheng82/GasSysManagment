...
<div class = "right_info">
    <div class="row" ng-controller="gasCollectionbyDistrictCtrl">
        <div class="col-md-3">
            <div class="panel panel-default">
                ...
                <div class="panel-body text-left">
                    <form class="form-horizontal" role="form">
                        <legend class="righter">区域选择</legend>
                        <div class="form-group">
                            <label for="province" class="col-md-3 control-label">公司</label>

                            <div class="col-md-9">
                                <select class="form-control input-sm"
                                        ng-options="key as value for (key , value) in provinces"
                                        ng-model="province" id="province"
                                        ng-change="province_update(province)">
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <select class="form-control input-sm"
                                        ...
                                        ng-model="city" id="city" ng-change="city_update(city)">
                                </select>
                            </div>
                        </div>
                ...
                <div class="panel-footer text-left">
                    <button type="button" id="search-button" class="btn btn-danger light btn-block compose-btn"
                            ng-click="district_update(district)">条件查询
                    </button>
                </div>
            </div>
        </div>
                ...
                <div class="panel-body text-left">
                    <div class="col-md-12">
                        <table st-table="displayedCollection" st-safe-src="rowCollection" class="table table-striped table-bordered">
                            <thead>
                            <tr>
                                <th st-sort="data_id">用户名称</th>
                                <th st-sort="data_vb">标况总累计(Nm³)</th>
                                <th st-sort="data_vm" ng-if="meter.meter_type != '卓度'">工况总累计(Nm³)</th>
                                <th st-sort="data_p" ng-if="meter.meter_type != '卓度'">压力(bar)</th>
                                <th st-sort="data_t" ng-if="meter.meter_type != '卓度'">温度(℃)</th>
                                <th st-sort="data_t" ng-if="meter.meter_type != '卓度'">用气曲线</th>
                                <th st-sort="data_vb">报表</th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr ng-repeat="row in displayedCollection">
                                <td>{{row.meter_company}}</td>
                                <td>{{row.meter_data_vb}}</td>
                                <td ng-if="meter.meter_type != '卓度'">{{row.meter_data_vm}}</td>
                                <td ng-if="meter.meter_type != '卓度'">{{row.meter_data_p}}</td>
                                <td ng-if="meter.meter_type != '卓度'">{{row.meter_data_t}}</td>
                                ...
                                <td ng-if="meter.meter_type != '卓度'">
                                    <div>
                                        <button type="button" ng-click="open(row)"
                                                class="btn btn-sm btn-info">生成报表
                                        </button>
                                    </div>
                                </td>
                            </tr>
