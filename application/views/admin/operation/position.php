    ...
    <form method="post" id="newpro_form" >
        <div class="ncsc-form-goods">
            ...
            <dl id="cycle_days">
                <dt>职位</dt>
                <dd class="rowform">
                    <input type="text" name="position"  id="position" maxlength="30" value=""/>
                </dd>
            </dl>
            <dl id="cycle_days">
                <dt>排序</dt>
...
<script>
    $(function () {
        ...
        function sendSearch(obj){
            sendPostData(obj, '<?php echo BASE_SITE_URL?>/admin/operation/positionList', function (res) {
                $('#list_content').html('');
                $('.pagination').html('');
                if (res.code == 1) {
                    showList(res.data);
        ...
        $('#list_content').on('click','.delnature',function(){
            ...
            sendPostData({id:id}, '<?php echo BASE_SITE_URL?>/admin/operation/deletePosition', function (res) {
                if(res.code == 1){
                    showTips('删除成功');
            ...
            if (data.rows && data.rows.length > 0) {
                ...
                for(var i = 0 ;i < data.rows.length;i++){
                    ...
                    str +=  '<td><span>'+obj.id+'</span></td>'+
                        '<td><span>'+obj.company_position+'</span></td>'+
        ...
        $('#newpro_form').validate({
            ...
            submitHandler: function () {
                ...
                sendPostData(data, '/admin/operation/addPosition', function (res) {
                    if(res.code == 1){
                        showTips('添加成功')
                        sendSearch(searchObj);
                        $('#position').val('');
