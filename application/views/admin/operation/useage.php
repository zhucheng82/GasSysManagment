    ...
    <form method="post" id="newpro_form" >
        <div class="ncsc-form-goods">
            ...
            <dl id="cycle_days">
                <dt>用途</dt>
                <dd class="rowform">
                    <input type="text" name="useage"  id="useage" maxlength="30" value=""/>
                </dd>
            </dl>
            <dl id="cycle_days">
                <dt>排序</dt>
...
<script>
    $(function () {
        ...
        function sendSearch(obj){
            sendPostData(obj, '<?php echo BASE_SITE_URL?>/admin/operation/useageList', function (res) {
                $('#list_content').html('');
                $('.pagination').html('');
                if (res.code == 1) {
                    showList(res.data);
        ...
        $('#list_content').on('click','.delnature',function(){
            ...
            sendPostData({id:id}, '<?php echo BASE_SITE_URL?>/admin/operation/deleteUseage', function (res) {
                if(res.code == 1){
                    showTips('删除成功');
        ...
        $('#newpro_form').validate({
            ...
            submitHandler: function () {
                ...
                sendPostData(data, '/admin/operation/addUseage', function (res) {
                    if(res.code == 1){
                        showTips('添加成功')
                        sendSearch(searchObj);
                        $('#useage').val('');
