...
(function () {
    ...
    window.qixiang.utils = function () {
        if (typeof window.qixiang.utils.instance === 'object') {
            return window.qixiang.utils.instance;
        }

        window.qixiang.utils.instance = this;
        var self = this;
        var debug = 1
        ...
        this.sendPostData = function (obj, url, fun, errfun, loading) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if ((res.code == -1001 || res.msg == '请先登录') || res.code == -1000) {
                        //var userType= self.getUrlParam('user_type');
                        //if(!userType){
                        //    userType = self.get_json_fromlocal('userType');
                        //}
                        //if(!userType){
                        //    userType = 1;
                        //}
                        self.save_json_tolocal('jumpURL',location.href);
        ...
        function get_sign_str(json_obj, time, need_token) {
            ...
            //alert(str);
            str += "appkey=lkjsdfk#@&&timestamp=" + time;
        ...
        this.previewImage = function(file, imageid, url,size,succfun,postData,efffun) {
            //var MAXWIDTH  = 100;
            //var MAXHEIGHT = 100;
        ...
        this.getTimePast = function (time){
            var now = new Date().getTime()/1000;
            var left = now - time;
            var h = parseInt(left/3600);
            var m = parseInt(left%3600/60);
            var s = (left%60);
            if(h > 0){
                return h+'小时前';
            }else{
        ...
        function isNeedToken(url) {
            if (url.indexOf('api/upload/img') >= 0) {
                return false;
            } else {
                return true;
            }
        ...
        this.addAnimate = function(element,rc,ac,hide,fun){
            $(element).show();
            $(element).addClass('animated');
            $(element).removeClass(rc).addClass(ac);
            var flag = false;
            $(element).one('webkitAnimationEnd oTransitionEnd webkitTransitionEnd msTransitionEnd animationend',function(){
                if(flag){//防止重复执行
                    return ;
                }
                flag = true;
        ...
        function whichTransitionEvent(){
            ...
            var t;
            if(!fakee){
                fakee = document.createElement('fakeelement');
            }

            for(t in transitionsEnd){

                if( fakee.style[t] !== undefined ){
...
;(function($){
    ...
    if (typeof define === 'function' && define.amd) {
        define(function () {
            return addTempDateFormat
        })
    } else if (typeof module === 'object' && module.exports) {
        module.exports = addTempDateFormat
    } else {
        $.addTempDateFormat = addTempDateFormat
    }
}(this));