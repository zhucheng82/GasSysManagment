...
var loading_content = '<div style="top:0px;left:0px;position: absolute;width: 100%;height: 100%;z-index: 10000;background: rgba(0,0,0,0.1);" id="loading_inmation">'
 ...
 * @copyright ZhouHr 2010-08-27
 */
function showTips( tips, time,fun ){
    if(!time){
        time = 2;
    }
...
function upload_file(imgId, imgType, timestamp, token,  fileSizeLimit, buttonText, fileTypeExts){
    ...
    $('#'+imgId+'_upload').uploadify({
        ...
        'onUploadSuccess' : function(file, data, response) {
            ...
            data = data.split('|');
            if (data[0] == 100){
                $('#'+imgId).nextAll('em').html('<i class="icoErr16"></i>'+data[1]);
            }else if(data[0] == 200 && data[1]!=''){
                var imgpath=data[1];
                $('#'+imgId).val(imgpath);
                $('#'+imgId).nextAll('em').html('<i class="icoCor16"></i>');
                $('#preview_'+imgId).attr('src','/'+imgpath);
            }
        }
...
function getKindEditor_small_group(arrText){
    var editor;
...
function getKindEditor_small(textName){
    var editor;
    KindEditor.ready(function(K) {
        K.create(textName, {
            resizeType : 1,
