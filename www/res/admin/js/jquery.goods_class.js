$(document).ready(function(){
    ...
    $('img[nc_type="flex"]').click(function(){
        ...
        if(status == 'open'){
            ...
            //ajax
            $.ajax({
                url: "/admin/category/ajax?parent_id="+id,
                dataType: 'json',
                success: function(data){
                    ...
                    for(var i = 0; i < data.length; i++){
                        ...
                        src += "<td class='w36'><input type='checkbox' name='check_gc_id[]' value='"+data[i].id+"' class='checkitem'>";
                        //图片
                        ...
                        //排序
                        src += " <span  ajax_branch='goods_class_sort' datatype='number' fieldid='"+data[i].id+"' fieldname='gc_sort' nc_type='inline_edit' >"+data[i].sort+"</span></td>";
                        //名称
                        ...
                        src += " <span  required='1' fieldid='"+data[i].id+"' ajax_branch='goods_class_name' fieldname='gc_name' nc_type='inline_edit' class='tooltip'>"+data[i].name+"</span>";
                        //新增下级
                        if(data[i].deep<3){
                            src += "<a class='btn-add-nofloat marginleft' href='"+ADMIN_SITE_URL+"/category/add?parent_id="+data[i].id+"'><span>新增下级</span></a>";
                        }
                        src += "</td>";
                        //是否显示
                        ...
                        //操作
                        src += "<td class='w84'>";
                        src += "<a href='"+ADMIN_SITE_URL+"/category/add?id="+data[i].id+"'>编辑</a>";
                        src += " | <a href=\"javascript:if(confirm('删除该分类将会同时删除该分类的所有下级分类，您确定要删除吗'))window.location = '"+ADMIN_SITE_URL+"/category/del?id="+data[i].id+"';\">删除</a>";
                        src += "</td>";
                        src += "</tr>";
                    }
                    //插入
