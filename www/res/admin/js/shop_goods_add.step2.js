$(function(){
    ...
    $('#goods_image').fileupload({
        dataType: 'json',
        url: SITEURL + '/admin/goods_add/image_upload',
        formData: {name:'goods_image'},
        add: function (e,data) {
        	$('img[nctype="goods_image"]').attr('src', RESURL + '/seller/images/loading.gif');
            data.submit();
        },
    ...
    $('.spec_file').find('input[type="file"]').unbind().bind('change', function(){
        var id = $(this).attr('id');
        ajaxFileUpload_step2(id);
    });

    $('a[nctype="spec_file_del"]').click(function(){
        var nctype = $(this).parent().find('img').attr('nctype');console.log(nctype);
        $(this).parent().find('img').attr('src', '');
        $('span[nctype=input_checkbox]').find('input[nctype='+nctype+']').val('');
    });
    ...
    $('a[nctype="show_image"]').unbind().ajaxContent({
        ...
        loaderType:"img",
        loadingMsg: RESURL + "/seller/images/loading.gif",
        target:'#demo'
    }).click(function(){
        $(this).hide();
    ...
    //商品颜色
    $('a[nctype="show_spec_image"]').unbind().ajaxContent({
        event:'click', //mouseover
        loaderType:"img",
        loadingMsg: RESURL + "/seller/images/loading.gif",
        target:'#spec_goods_demo'
    }).click(function(){
        $(this).hide();
        $('a[nctype="spec_goods_demo"]').show();
    });
    $('a[nctype="spec_goods_demo"]').unbind().click(function(){
        $('#spec_goods_demo').html('');
        $(this).hide();
        $('a[nctype="show_spec_image"]').show();
    });


    $('#add_album').fileupload({
        dataType: 'json',
        url: SITEURL + '/admin/goods_tpl_add/image_upload',
    ...
    // 计算折扣
    $('input[name="price"],input[name="market_price"]').change(function(){
        discountCalculator();
    });
    ...
    $('.letter[nctype="letter"]').find('a[data-letter]').click(function(){
        ...
        $.getJSON(_url, {type : 'letter', category_id : _tid, letter : _letter}, function(data){
    ...
    $('.search[nctype="search"]').find('a').click(function(){
        ...
        $.getJSON(_url, {type : 'keyword', category_id : _tid, keyword : _keyword}, function(data){
    ...
    $('a[nctype="mb_add_img"]').click(function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('/seller/goods_album/pic_list?item=mobile');
    });
    // 关闭 图片选择
 ...
 // 图片上传ajax
function ajaxFileUpload_step2(id, o) {
    $('#'+id).fileupload({
        dataType: 'json',
        url: SITEURL + '/admin/goods_tpl_add/image_upload',
        formData: {name:id},
        add: function (e,data) {
            $('img[nctype="'+id+'"]').attr('src', RESURL + '/seller/images/loading.gif');
            data.submit();
        },
        done: function (e,data) {
            var param = data.result;
            if (typeof(param.error) != 'undefined') {
                ...
                $('input[nctype="'+id+'"]').val(param.name);
                $('img[nctype="'+id+'"]').attr('src',param.thumb_name);
            }
        }
    });
...
function computeStock(){
    ...
    $('input[name="stock_num"]').val(_stock);
}
...
function computePrice(){
    ...
    $('input[name="price"]').val(number_format(_price, 2));
...
function discountCalculator() {
    ...
    var _marketprice = parseFloat($('input[name="market_price"]').val());
...
/* 插入商品color图片 */
function insert_img_color(name, src) {
    var $_thumb = $('#spec_img').find('td[nc_type=spec_img_td]');
    $_thumb.each(function(){
        var nctype = $(this).find('img').attr('nctype');
        var obj = $('span[nctype=input_checkbox]').find('input[nctype='+nctype+']');
        if (obj.val() == '') {
...
function insertBrand(param, search) {
    ...
    $.each(param, function(i, n){
        $('<li data-id="' + n.id + '" data-name="' + n.name + '"><em>' + n.initial + '</em>' + n.name + '</li>').appendTo(_ul);
    });
